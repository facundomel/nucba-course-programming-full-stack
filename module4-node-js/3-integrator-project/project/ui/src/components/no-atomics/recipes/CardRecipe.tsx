...
import * as recipeActions from "../../../redux/recipes/RecipesActions";
...
const CardRecipe = ({ id, name, description, img, publisher, ingredients, instructions }: CardRecipeProps) => {
	...
	const recipeIndex = recipesAll.findIndex((recipe: any) => recipe.id == id);
	...
	const handlerOnClickStar = () => {
		if (!recipesAll[recipeIndex].isFavorite) {
			...
			if (userSection == "MyRecipes") {
				dispatch(recipeActions.setRecipesFiltered(recipesAll.filter((recipe: any) => recipe.isFavorite)));
			}
		} else {
			dispatch(
				snackbarActions.setOptionsSnackbar({
